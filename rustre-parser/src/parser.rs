...
impl<'a, 'f> Parser<'a, 'f> {
    ...
    fn parse_id(&mut self, toks: &'a [Tok<'a, 'f>]) -> SpannedRes<'a, 'f, Ident<'a, 'f>> {
        ...
        if self.expect(&toks[1..], TokInfo::DoubleColon, "::").is_ok() {
            ...
            Ok((toks, id.clone().map(|_| Ident::Short { id, pragmas })))
        }
    }
    ...
    fn parse_static_arg(&mut self, toks: &'a [Tok<'a, 'f>]) -> Res<'a, 'f, StaticArg<'a, 'f>> {
        ...
        } else if self.expect(toks, TokInfo::Node, "").is_ok()
            || self.expect(toks, TokInfo::Function, "").is_ok()
        {
            let (
                toks,
                Spanned {
            ...
            ) = self.parse_effective_node(&toks[1..])?;
